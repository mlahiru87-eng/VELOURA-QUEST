...
export const Navbar: React.FC = () => {
  ...
  return (
    <>
      ...
      <header id="desktop-header" className="sticky top-0 z-40 bg-slate-950/85 backdrop-blur-xl border-b border-rose-950/50 px-4 sm:px-8 py-3 flex items-center justify-between">
        <div 
          onClick={() => setCurrentPage('home')}
          className="flex items-center space-x-3 cursor-pointer group"
        >
          <div className="w-10 h-10 rounded-xl electric-gradient-btn flex items-center justify-center text-white glow-red shadow-lg transition-transform group-hover:scale-105">
            ...
            return (
              <button
                ...
                className={`relative flex items-center space-x-2 px-3.5 py-2 rounded-xl text-xs font-semibold transition-all ${
                  isActive 
                    ? 'bg-rose-600/20 text-rose-300 border border-rose-500/40 glow-red' 
        ...
        <div className="flex items-center space-x-3">
          {userProfile && (
            <div 
              ...
              className="flex items-center space-x-2 px-3 py-1.5 rounded-full bg-slate-900/90 border border-rose-500/30 text-xs font-bold text-slate-100 cursor-pointer hover:border-rose-400 transition-all glow-red"
            >
          ...
          {userProfile ? (
            <button
              ...
              className="w-9 h-9 rounded-full overflow-hidden border border-rose-500/50 p-0.5 glow-red focus:outline-none"
            >
              <img 
      ...
      <div id="mobile-bottom-nav" className="md:hidden fixed bottom-0 left-0 right-0 z-50 bg-slate-950/95 backdrop-blur-2xl border-t border-rose-950/60 px-3 py-2 flex items-center justify-around">
        {navItems.map((item) => {
          const isActive = currentPage === item.page;
          return (
            <button
              ...
              className={`relative flex flex-col items-center justify-center p-1.5 rounded-xl transition-all ${
                isActive ? 'text-rose-400 font-bold scale-110' : 'text-slate-400 hover:text-slate-200'
              }`}
            >
              {item.icon}
              <span className="text-[10px] mt-0.5 tracking-tight">{item.label}</span>
              {item.badge && item.badge > 0 ? (
                <span className="absolute top-0 right-1 w-2.5 h-2.5 rounded-full bg-rose-600 ring-2 ring-slate-950 animate-ping"></span>
              ) : null}
            </button>
          );
        })}
